...
Рекордер экрана для Windows с архитектурой **Metadata-First**. Записывайте экран, автоматически генерируйте плавные зум-анимации на основе телеметрии кликов, редактируйте таймлайн и экспортируйте готовое MP4 — без повторной записи.
...
1. **Запись** — захват чистого видеопотока (без курсора) через Windows Graphics Capture с параллельным логированием телеметрии мыши/клавиатуры и контекста UI-элементов.
2. **Редактирование** — автоматическая генерация зум-сегментов из кластеров кликов и рендер виртуального курсора. Настройка зон зума, тайминга, размера курсора и фона на визуальном таймлайне.
3. **Экспорт** — композиция финального видео через Remotion и кодирование в MP4 через FFmpeg.
...
- Авто-зум из кластеризации кликов с easing-переходами и lookahead
...
- Превью композиции в реальном времени
...
| Композиция видео | [Remotion](https://www.remotion.dev/) |
...
3. **Экспорт** — задайте параметры и нажмите Export. Финальный MP4 рендерится с плавными зумами и виртуальным курсором.
